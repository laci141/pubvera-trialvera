...
func main() {
	...
	time.Sleep(d)
	out, _ := json.Marshal(map[string]any{"status": "ok"})
...
func postSystemHealth(t *testing.T) (int, string, string) {
	t.Helper()
	body := `{"command":"health"}`
	req := httptest.NewRequest(http.MethodPost, "/api/system", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	handleGroup(rec, req, "system")
	return rec.Code, rec.Body.String(), rec.Header().Get("Retry-After")
}

func fireHealthRequests(t *testing.T, n int) []int {
	t.Helper()
	codes := make([]int, n)
...
func TestCLISemaphoreBoundsChildRuns(t *testing.T) {
	...
	codes := fireHealthRequests(t, requests)
	bounded := peakConcurrency(t, boundedLog)

	t.Logf("MEASURED peak concurrent child processes for %d requests: unbounded=%d bounded=%d",
...
func TestCLISemaphoreRejectsWithRetryAfter(t *testing.T) {
	...
	code, body, retryAfter := postSystemHealth(t)
	cliSem.release()
